...
def load_ground_truth(label_folder):
    ...
    for label_file in sorted(os.listdir(label_folder)):
        # Trích xuất số frame từ tên file (ví dụ: frame_0000.jpg.rf.<hash>.txt -> 0)
        if label_file.startswith('frame_') and label_file.endswith('.txt'):
            try:
                # Lấy phần frame_XXXX từ tên file
                frame_part = label_file.split('.')[0]  # Lấy frame_XXXX
                frame_str = frame_part.split('_')[1]  # Lấy phần số (XXXX)
                frame_idx = int(frame_str)  # Chuyển thành số nguyên
            ...
            with open(os.path.join(label_folder, label_file), 'r') as f:
                for line in f:
                    ...
                    if class_id == 0:  # Chỉ lấy class person
                        ...
                        x_min = (cx - w/2) * 960  # Video width = 960
                        y_min = (cy - h/2) * 540  # Video height = 540
                        x_max = (cx + w/2) * 960
                        y_max = (cy + h/2) * 540
