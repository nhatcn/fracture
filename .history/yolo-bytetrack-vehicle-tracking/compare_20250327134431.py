...
LABELS_FOLDER = "train/labels"  # Path to your label files
MAX_FRAMES = 5252  # Total frames in your video
...
def load_ground_truth_labels(labels_folder, max_frames):
    ...
    for label_file in label_files:
        # Extract frame number from filename (e.g., test2_mp4-0003_jpg.rf.<hash>.txt)
        match = re.search(r'-(\d+)_jpg\.rf\.', label_file)
        if match:
            # Extract the frame number and convert to integer
            frame_num = int(match.group(1))
            # Adjust for 0-based indexing (e.g., 0003 in filename -> frame 2)
    ...
    for frame_idx, label_file in label_files_with_frame:
        ...
        with open(label_path, 'r') as f:
            ...
            # Extract bounding boxes (center_x, center_y, width, height) and class IDs
            boxes = [[float(x) for x in label[1:]] for label in labels]
    ...
    print(f"Labeled frames: {labeled_frames[:10]}... (first 10 frames)")
...
if __name__ == "__main__":
    ...
    labeled_frames_set = set(labeled_frames)

    # Perform inference only on labeled frames
    predictions_best5 = {}
    predictions_yolov8m = {}
    ...
    for frame_idx, frame in enumerate(generator):
        if frame_idx >= MAX_FRAMES:
            break
        if frame_idx not in labeled_frames_set:
            continue  # Skip frames that don't have labels
        print(f"Processing labeled frame {frame_idx + 1} (0-based index: {frame_idx})")
        # Best5 predictions
        ...
        print(f"Frame {frame_idx + 1} - Best5 detections (class 0 - pedestrian): {len(det_best5.xyxy)}")
        ...
        print(f"Frame {frame_idx + 1} - YOLOv8m detections (class 0 - person): {len(det_yolov8m.xyxy)}")
    ...
    # Process video output for Best5
    ...
    # Process video output for YOLOv8m
