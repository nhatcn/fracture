...
class ResBlock_CBAM(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv2d(in_channels, out_channels // 4, kernel_size=1, bias=False),  # bottleneck.0
            nn.BatchNorm2d(out_channels // 4),  # bottleneck.1
            nn.ReLU(inplace=True),
            nn.Conv2d(out_channels // 4, out_channels // 4, kernel_size=3, stride=stride, padding=1, bias=False),  # bottleneck.3
            nn.BatchNorm2d(out_channels // 4),  # bottleneck.4
            nn.ReLU(inplace=True),
            nn.Conv2d(out_channels // 4, out_channels, kernel_size=1, bias=False),  # bottleneck.6
            nn.BatchNorm2d(out_channels),  # bottleneck.7
            nn.ReLU(inplace=True)
        )
        # Tầng CBAM
